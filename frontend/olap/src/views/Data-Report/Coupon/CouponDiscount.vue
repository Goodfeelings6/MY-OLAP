...
export default {
  ...
  methods: {
    ...
    load1(dt)// main1
    {
      ...
      option = {
        ...
        tooltip: {
          ...
          formatter: function (param) {
            ...
            param = param[0]
            if (param.data) {
              return param.marker + "优惠劵名称 : " + param.name
                + " <br/>  优惠劵补贴率 : " + param.value
                + " <br/>  发布日期 : " + param.data.start_time
                + " <br/>  优惠规则 : " + param.data.rule;
            } else return null;
          },
          axisPointer: {
            type: 'cross',
        ...
        series: [
          {
            ...
            data: [
              { value: 0.1, start_time: "2020-06-12", rule: "满1000减5" },
              { value: 0.6, start_time: "2020-06-10", rule: "满20减2" },
              { value: 0.5, start_time: "2020-06-11", rule: "满200减10" },
              { value: 0.7, start_time: "2020-06-19", rule: "满100减20" },
              { value: 0.3, start_time: "2020-06-04", rule: "满50减10" },
            ]
          }
        ]
      };
      ...
      // 请求数据
      this.request.post(this.ip + "/coupon/discount", {
        dt: dt
      }).then(res => {
        console.log("请求ip", this.ip + "/coupon/discount", "成功", res)
        var axis = []
        var mydata = []
        res.forEach(obj => {
          axis.push(obj.coupon_name);
          var tmp = {}
          tmp.value = obj.reduce_rate;
          tmp.start_time = obj.start_date;
          tmp.rule = obj.rule_name;
          mydata.push(tmp);
        });
        ...
        console.log("err,请求ip", this.ip + "/coupon/discount")
      })
    },
    // 切换日期
    checkout_date() {
